...
def home(request):
	...
	b =  AudioSegment.from_mp3('اهلا.mp3')
	msg= play(b)
	return render(request,'index.html',{'data':data,'banners':banners,'cat':cat,'msg':msg})

# Category
# def category_list(request):
#     data=Category.objects.all().order_by('-title')
...
def product_list(request):
	...
	data=Product.objects.all().order_by('title')
	cat=Category.objects.all().order_by('id')
...
def product_detail(request,slug,id):
	...
	related_products=Product.objects.filter(is_featured=True).order_by('-id')
	cat=Category.objects.all().order_by('id')
	brand=Brand.objects.all().order_by('id')
	...
	return render(request, 'product-detail.html',{'data':product,'related':related_products,'colors':colors,'sizes':sizes,'reviewForm':reviewForm,'canAdd':canAdd,'reviews':reviews,'avg_reviews':avg_reviews,'brand':brand,'cat':cat,})
...
def cart_list(request):
	...
	if request.method == 'POST':
			...
			contact=CartOrder(name=name,number=number)
			contact.save()
...
def delete_cart_item(request):
	...
	for p_id,item in request.session['cartdata'].items():
		total_amt+=int(item['qty'])*float(item['price'])
	t=render_to_string('cartlist.html',{'cart_data':request.session['cartdata'],'totalitems':len(request.session['cartdata']),'total_amt':total_amt})
...
def checkout(request):
	...
	if 'cartdata' in request.session:
		b =  AudioSegment.from_mp3('as.mp3')
		play(b)
		msg= play
		for p_id,item in request.session['cartdata'].items():
		...
		return render(request, 'checkout.html',{'cart_data':request.session['cartdata'],'totalitems':len(request.session['cartdata']),'total_amt':total_amt,'form':form,'address':address,'msg':msg})
